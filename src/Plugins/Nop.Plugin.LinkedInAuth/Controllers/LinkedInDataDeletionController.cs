...
{
    public class LinkedInDataDeletionController : Controller
    {
        #region Fields
        ...
        public LinkedInDataDeletionController(LinkedInExternalAuthSettings LinkedInExternalAuthSettings,
        ...
        public async Task<IActionResult> DataDeletionCallback(IFormCollection form)
        {
            try
            {
                ...
                var appSecretBytes = Encoding.UTF8.GetBytes(_LinkedInExternalAuthSettings.ClientSecret);
                ...
                var fbUser = JsonConvert.DeserializeObject<LinkedInUserDTO>(json);
                var authenticationParameters = new ExternalAuthenticationParameters
                {
                    ProviderSystemName = LinkedInAuthenticationDefaults.SystemName,
                    AccessToken = await HttpContext.GetTokenAsync(OAuthDefaults.DisplayName, "access_token"),
                ...
                {
                    await _logger.InformationAsync($"{LinkedInAuthenticationDefaults.SystemName} data deletion completed. " +
                ...
                var notificationUrl = Url.RouteUrl(LinkedInAuthenticationDefaults.DataDeletionStatusCheckRoute,
            ...
            {
                await _logger.ErrorAsync($"{LinkedInAuthenticationDefaults.SystemName} data deletion error: {exception.Message}.", exception);
                return BadRequest();
            }
        }
